  ...
  render() {
    ...
    const versionReleases =
      ...
      Object.keys(time)
        ...
        .filter((t) => Number.parseInt(t.version) >= 0);
    const averageReleaseInDays =
      time &&
      Number.parseInt(
        versionReleases.map((v) => v.time).reduce((a, b) => a + b) /
          versionReleases.length,
      );

    return (
      <div
        style={{ padding: "20px", margin: "0 auto", width: "80%" }}
        className="grid"
      >
        <div
          className="col-7-12"
          style={{ borderRight: "1px solid #e9e9e9", paddingRight: "15px" }}
        >
          <div
            style={{
              minHeight: "100px",
          ...
          >
            <a className="btn" href="/_browse">
              {" "}
              <span className="fa fa-arrow-left" /> Back to Search{" "}
            </a>
            <br />
            <br />
            <br />
              ...
              {keywords && keywords.length > 0 ? (
                <div style={{ marginTop: "10px" }}>
                  <span className="fa fa-tag">
                    {" "}
                    <small> {keywords.slice(0, 4).join(", ")} </small>{" "}
                  </span>
                </div>
              ) : (
                ""
        ...
        <div className="col-5-12" style={{ paddingLeft: "15px" }}>
          ...
          {latest ? (
            <p>
              ...
              <b>{moment(doc.time[latest]).fromNow()}</b>{" "}
            </p>
          ) : (
            ""
          )}
          ...
          {homepage ? (
            <a
              ...
              style={{ margin: "10px 0", padding: "1% 2%", width: "95%" }}
            >
          ...
          {time ? (
            <div>
              ...
              >
                {" "}
                Version Release (<small>{averageReleaseInDays} day avg </small>){" "}
              </p>
              <div style={{ padding: "10px 0" }}>
                <LineChart
                  width={window.innerWidth * 0.8 * (5 / 12)}
                  height={250}
                  data={versionReleases}
